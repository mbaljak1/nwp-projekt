<?php 
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'edit_news') {
        ...
        if($_FILES['picture']['error'] == UPLOAD_ERR_OK && $_FILES['picture']['name'] != "") {
			...
			if ($ext == '.jpg' || $ext == '.png' || $ext == '.gif') { # test if format is picture
				...
				$_result = @mysqli_query($MySQL, $_query);
				$_SESSION['message'] .= '<p>Usjpješno ste dodali sliku.</p>';
			}
        }
		
		$_SESSION['message'] = '<p>Uspješno ste promjenili vijest!</p>';
	...
	#Edit news
		...
		<div id="news">
			...
			<a href="index.php?menu=' . $menu . '&amp;action=' . $action . '&amp;add=true" class="AddLink">Dodaj još vijesti</a>
		</div>';
	}
